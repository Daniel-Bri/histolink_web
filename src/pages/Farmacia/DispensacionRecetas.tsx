...
export default function DispensacionRecetas() {
    ...
    const recetasFiltradas = recetas.filter(r => {
        ...
        return (
            ...
            r.paciente_ci.toLowerCase().includes(q) ||
        ...
        <div style={{ padding: '32px' }}>
            ...
            <div style={{
                ...
                <input
                    ...
                    placeholder="Buscar por número de receta, nombre o CI del paciente..."
                    style={{
                            ...
                            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                                <thead>
                                    <tr style={{ background: '#F0F6FF' }}>
                                        {['N° Receta', 'Paciente', 'Medicamentos', 'Estado', 'Fecha emisión', ''].map(h => (
                                            <th key={h} style={{
                                    ...
                                    {pendientes.map(r => (
                                        <tr key={r.id} style={{ borderTop: '1px solid #F0F6FF' }}>
                                            ...
                                            <td style={{ padding: '10px 14px', fontSize: '13px', color: '#333' }}>
                                                <span style={{ fontWeight: 600 }}>{r.paciente_nombre}</span>
                                                <br />
                                                <span style={{ fontSize: '11px', color: '#64748B' }}>CI {r.paciente_ci}</span>
                                            </td>
                                            <td style={{ padding: '10px 14px', fontSize: '13px', color: '#333' }}>
                    ...
                    <div style={{
                        ...
                        {dispensadas.length === 0 ? (
                            ...
                            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                                <thead>
                                    <tr style={{ background: '#F0F6FF' }}>
                                        {['N° Receta', 'Paciente', 'Medicamentos', 'Estado', 'Fecha dispensación', ''].map(h => (
                                            <th key={h} style={{
                                                padding: '10px 14px', textAlign: h === '' ? 'right' : 'left',
                                                fontSize: '12px', fontWeight: 700, color: '#0003B8', textTransform: 'uppercase',
                                            }}>{h}</th>
                                                    ...
                                                    <td colSpan={6} style={{ padding: '12px 20px' }}>
                                                        <div style={{ fontSize: '12px', color: '#333' }}>
                                                            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '6px' }}>
                                                                <p style={{ margin: 0, fontWeight: 700, color: '#0003B8' }}>
                                                                    ⛓ Evento Blockchain — Bloque #{blockchainData[r.id].evento_blockchain.numero_bloque}
                                                                </p>
                                                                <span style={{
                                                                    background: blockchainData[r.id].es_integro ? '#DCFCE7' : '#FEE2E2',
                                                                    color: blockchainData[r.id].es_integro ? '#166534' : '#DC2626',
                                                                    border: `1px solid ${blockchainData[r.id].es_integro ? '#BBF7D0' : '#FECACA'}`,
                                                                    padding: '2px 10px', borderRadius: '9999px', fontSize: '11px', fontWeight: 700,
                                                                }}>
                                                                    {blockchainData[r.id].es_integro ? '✓ Íntegro' : '✗ Alterado'}
                                                                </span>
                                                            </div>
